...
GETBIJI_BASE_URL = os.environ.get("GETBIJI_BASE_URL", "").strip()  # 注意：去掉 rstrip("/")
...
def validate_config():
    ...
    if not GETBIJI_API_KEY:
        errors.append("GETBIJI_API_KEY 未设置")
    
    if not GETBIJI_CLIENT_ID:
        errors.append("GETBIJI_CLIENT_ID 未设置")
    
    if not GETBIJI_BASE_URL:
        errors.append("GETBIJI_BASE_URL 未设置")
    elif not GETBIJI_BASE_URL.startswith(("http://", "https://")):
        errors.append(f"GETBIJI_BASE_URL 格式错误，应该以 http:// 或 https:// 开头: {GETBIJI_BASE_URL}")
    elif "openapi.biji.com" not in GETBIJI_BASE_URL:
        log_warning(f"GETBIJI_BASE_URL 不包含 openapi.biji.com: {GETBIJI_BASE_URL}")
    
    if not NOTION_TOKEN:
        errors.append("NOTION_TOKEN 未设置")
    
    if not NOTION_DATABASE_ID:
        errors.append("NOTION_DATABASE_ID 未设置")
...
def main():
    ...
    required_vars = {
        ...
        "GETBIJI_BASE_URL": (GETBIJI_BASE_URL, 50),  # 显示更多字符以便调试
    ...
    if not validate_config():
        log_error("配置验证失败，同步终止")
        sys.exit(1)
    
    # 输出完整的 Base URL 用于调试
    log_info(f"完整的 GETBIJI_BASE_URL: {GETBIJI_BASE_URL}")
